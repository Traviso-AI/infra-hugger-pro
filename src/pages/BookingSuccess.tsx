  ...
  return (
    <div className="container max-w-lg px-4 py-10 md:py-16">
        ...
        >
          {Array.from({ length: 20 }).map((_, i) => (
            <motion.div
              ...
              animate={{ x: (Math.random() - 0.5) * 400, y: (Math.random() - 0.5) * 400, scale: [0, 1, 0.5], rotate: Math.random() * 720 }}
              transition={{ duration: 1.5, delay: i * 0.05, ease: "easeOut" }}
              className="absolute h-3 w-3 rounded-full"
              style={{ backgroundColor: ["#29A38B", "#FFD700", "#FF6B6B", "#4ECDC4", "#7C3AED"][i % 5] }}
            />
          ))}
        </motion.div>
      )}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center mb-8">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", damping: 10, delay: 0.3 }}>
        ...
        <p className="text-muted-foreground">Your trip to <strong className="text-foreground">{destination}</strong> is all set.</p>
      </motion.div>
        ...
        <Card className="overflow-hidden mb-6">
          {hotels[0]?.image_url && (
            <div className="h-40 w-full overflow-hidden">
              <img src={hotels[0].image_url} alt={destination} className="h-full w-full object-cover" />
            </div>
          )}
          <CardContent className="p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">{destination}</h2>
              <Badge className="bg-accent text-accent-foreground"><Sparkles className="h-3 w-3 mr-1" />Confirmed</Badge>
            </div>
            <div className="space-y-2.5">
              {flightRef && (
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center gap-2 text-muted-foreground"><Plane className="h-4 w-4" />Flight Reference</span>
              ...
              {hotels[0]?.check_in_date && (
                <div className="flex items-center justify-between text-sm">
                  <span className="flex items-center gap-2 text-muted-foreground"><Calendar className="h-4 w-4" />Dates</span>
      ...
      {commission && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="mb-6 p-4 rounded-xl bg-accent/5 border border-accent/15">
        ...
        <Card className="mb-6">
          <CardContent className="p-5">
            ...
            <div className="grid grid-cols-4 gap-2">
              <Button variant="outline" size="sm" className="text-xs" onClick={() => handleShare("instagram")}>Instagram</Button>
              <Button variant="outline" size="sm" className="text-xs" onClick={() => handleShare("tiktok")}>TikTok</Button>
              <Button variant="outline" size="sm" className="text-xs" onClick={() => handleShare("whatsapp")}>WhatsApp</Button>
              <Button variant="outline" size="sm" className="text-xs" onClick={handleCopyLink}><Copy className="h-3 w-3 mr-1" />Link</Button>
            </div>
          </CardContent>
        </Card>
      </motion.div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="flex gap-3">
        <Button variant="outline" className="flex-1" onClick={() => navigate("/dashboard")}>View My Bookings</Button>
        <Button className="flex-1 bg-accent text-accent-foreground hover:bg-accent/90" onClick={() => navigate("/explore")}>
          <ExternalLink className="h-4 w-4 mr-1.5" />Explore More
        </Button>
      </motion.div>
    </div>
  );
